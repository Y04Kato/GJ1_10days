...
namespace ParticleIbu {
	...
	public:
		...
		void Dispach(ParticleIbu::GpuParticle* particle);
...
#pragma endregion
	...
	template<typename T>
	inline void ParticleField<T>::CreateType(std::string name)
	{
		name_ = name;
		param_.resize(max_);
	...
	template<typename T>
	inline void ParticleField<T>::Dispach(ParticleIbu::GpuParticle* particle)
	{
		SPSOProperty pso;
		if constexpr (std::is_same<T, ParticleIbu::FieldType::FieldSuction>::value)
		{
